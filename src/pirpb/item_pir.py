...
# item_pir.py
# ==========================================================
# Item-level, rank-based PIR (NO RBO).
#
# For each seed query with a defined focal item:
#   1. Determine a baseline rank for the focal item:
#        - Prefer seed query SERP if focal appears in top-k.
#        - Otherwise, use the best (lowest) rank of the focal
#          item across that seed's paraphrases (anchor).
#        - If focal never appears, skip this seed.
#   2. For each paraphrase of that seed:
#        - Find para_rank of focal item.
#        - Compute delta_rank = para_rank - baseline_rank.
#        - impact_score in [0,1]:
#              * Only penalize demotions / disappearance.
#              * Promotions or equal -> 0.
#              * Missing focal -> treated as rank = k+1.
#
# Also attaches similarity_bin for each paraphrase so we can
# analyze how impact changes with perturbation strength.
#
# Inputs:
#   - data/queries/base_queries.csv
#   - data/queries/focal_items.csv
#   - data/paraphrases/paraphrases.csv
#   - serp/serp_google.csv
#
# Outputs:
#   - results/item_pir_google.csv
#   - results/item_pir_summary.csv
#       (mean impact per category & similarity_bin)
# ==========================================================
...
def find_rank_for_target(
    ...
    """
    Return rank (1..k) of focal item for given query_text,
    or k+1 if not found / no rows.
    """
...
def compute_item_pir(
    ...
    if "similarity_bin" not in paras_df.columns:
        raise ValueError(
            "paraphrases.csv must contain 'similarity_bin'. "
            "Run updated paraphraser first."
        )

    serp_df.columns = [c.lower() for c in serp_df.columns]
    if not {"query", "rank", "url"}.issubset(serp_df.columns):
        raise ValueError("serp_google.csv must have 'query', 'rank', 'url'.")
    ...
    for (seed_query, seed_cat_l), (f_type, f_val) in focal_lookup.items():
        ...
        pgrp = paras_df[paras_df["query"] == seed_query].copy()
        if pgrp.empty:
            print(
        ...
        # 2) if seed baseline not informative, fall back to best paraphrase
        if baseline_rank == k + 1:
            best_rank = k + 1
            best_q = None
            for _, prow in pgrp.iterrows():
                pq = str(prow["paraphrase"]).strip()
                if not pq:
                    continue
        ...
        # 3) if still no focal found anywhere, skip this seed
        if baseline_rank == k + 1:
            skipped_no_focal += 1
            continue

        # 4) compute impact for each paraphrase vs baseline
        for _, prow in pgrp.iterrows():
            para_q = str(prow["paraphrase"]).strip()
            if not para_q:
                continue
    ...
    if not item_rows:
        raise ValueError(
            ...
            "Likely missing focal appearances in serp_google.csv."
        )

    item_df = pd.DataFrame(item_rows)

    # summary: mean impact per (category, similarity_bin)
    summary_df = (
    ...
    print(f"[OK] Item-level rank PIR -> {item_output}")
    print(f"[OK] Item-level PIR summary -> {summary_output}")
...
def main() -> None:
    ...
    print(f"[INFO] Using top_k = {k} for item-level rank PIR.")
